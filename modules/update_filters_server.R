...
filter_specs <- list(
  ...
  list(id = "season", col = "season"),              # Added
  list(id = "function_derivation", col = "function_derivation") # Added
)
...
debounced_watch <- to_watch %>% debounce(300)
...
observeEvent(debounced_watch(), {
  ...
  for (spec in filter_specs) {
    # Calculate subset of valid data based on ALL OTHER selections
    df_sub <- data
    ...
    # CRITICAL: Switch server to FALSE to stop focus-loss glitches
    # Using freezeReactiveValue prevents the change from triggering itself instantly
